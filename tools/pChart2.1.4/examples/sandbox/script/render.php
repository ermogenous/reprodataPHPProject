 ...
 */
 ...
 $fontWholePath = 'http://localhost/reprodata/tools/pChart2.1.4';
 ...
 if ( $g_title_enabled == "true" )
  {
   if ( $Mode == "Render" )
    $myPicture->setFontProperties(array("FontName"=>$fontWholePath."/fonts/".$g_title_font,"FontSize"=>$g_title_font_size));
   else
 ...
 if ( $Mode == "Render" )
  $myPicture->setFontProperties(array("R"=>$R,"G"=>$G,"B"=>$B,"FontName"=>$fontWholePath."/fonts/".$s_font,"FontSize"=>$s_font_size));
 else
 ...
 if ( $c_display_values == "true" ) { $Config = array("DisplayValues"=>TRUE); } else { $Config = array(); }
 ...
 if ( $l_enabled == "true" )
  {
   ...
   $Config["FontName"] = $fontWholePath."/fonts/".$l_font;
