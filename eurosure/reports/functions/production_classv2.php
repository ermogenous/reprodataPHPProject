...
class synthesis_production {
	...
	public function synthesis_production() {
	
	$this->sql_select = "
1 as remove_clo_default_sort
";
	$this->sql_from = "
ydinproductionextendedloadings
";
	$this->sql_where = "1=1
	";
...
remove_clo_default_sort ASC";	
	...
	public function as_at_sql () {
		...
		JOIN ydinproductionextendedloadings ON inpva_policy_serial = ydpxl_financial_policy_abs
		JOIN inpolicies ON ydpxl_financial_policy_abs = inpol_policy_serial';
...
function add_clients() {
	...
	if ($this->added_tables["inclients"] != 1) {
		...
		$this->insert_from("JOIN inclients ON ydpxl_client_serial = incl_client_serial",1);	
	}
}
...
function add_insurance_types() {
	...
	if ($this->added_tables["ininsurancetypes"] != 1) {
		...
		$this->insert_from("JOIN ininsurancetypes ON inity_insurance_type_serial = ydpxl_insurance_type_serial",1);	
	}
...
function add_agents() {
	if ($this->added_tables["inagents"] != 1) {
		...
		$this->insert_from("JOIN inagents ON inag_agent_serial = ydpxl_agent_serial",1);	
	}
}
...
function add_situations() {
	if ($this->added_tables["inpolicysituations"] != 1) {
		...
		$this->insert_from("JOIN inpolicysituations ON inpst_policy_serial = ydpxl_situation_serial",1);
	}
}
...
function add_policy_items() {
	if ($this->added_tables["inpolicyitems"] != 1) {
		...
		$this->insert_from("JOIN inpolicyitems ON ydpxl_pit_auto_serial = inpit_pit_auto_serial",1);
	}
}
...
public function add_ydpxl_phase_premiums() {
	if ($this->add_ydpxl_phase_premiums != 1) {
		$this->add_ydpxl_phase_premiums = 1;
	
		$this->insert_select("SUM(ydpxl_loading_premium)",'Phase_Premium');
...
function generate_sql() {
	...
	$this->insert_where("AND ydpxl_process_status in (".$this->clo_process_status.")");
	$this->insert_where("AND ydpxl_policy_status in (".$this->clo_status.")");
