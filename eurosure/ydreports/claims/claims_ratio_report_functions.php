<?
function get_data_for_area($area,$cymenu,$section,$month,$year,$as_at_date,$agents_from,$agents_to,$other_sql='') {
...
foreach ($all_sections as $section) {
		//get agents
		if ($agents_to != "") {
			$agents_sql = "AND ap_agents_code BETWEEN '".$agents_from."' AND '".$agents_to."'";
		}
		else {
			$agents_sql = '';
		}
		
		$ag = $db->query("SELECT * FROM ap_agents WHERE ap_active = 1 AND ap_location LIKE '".$area."' ".$agents_sql." ORDER BY ap_agents_code");

		while ($agent = $db->fetch_assoc($ag)) {
		
			$data[$agent["ap_agents_ID"]][$section]["code"] = $agent["ap_agents_code"];
			$data[$agent["ap_agents_ID"]][$section]["code_old"] = $agent["ap_agents_code_old"];
			$data[$agent["ap_agents_ID"]][$section]["name"] = $agent["ap_name"];
			$data[$agent["ap_agents_ID"]][$section]["section"] = $section;
			
			//production for cymenu
			if ($section == 'ALL') {
				$section_sql = "";
			}
			else if ($section == 'NON') {
			...
			$data[$agent["ap_agents_ID"]][$section]["premium"] = $premium["clo_total_premium"];
			$data[$agent["ap_agents_ID"]][$section]["fees"] = $premium["clo_fees"];
			$data[$agent["ap_agents_ID"]][$section]["commission"] = $premium["clo_total_commission"];
			$data[$agent["ap_agents_ID"]][$section]["stamps"] = $premium["clo_stamps"];
			$data[$agent["ap_agents_ID"]][$section]["mif"] = $premium["clo_mif"];
			
			//production for synthesis
			if ($section == 'ALL') {
				$section_sql = "%%";
				$section_way = 'LIKE';
			}
			...
			$ssql = $sybase->query($inqueries->agent_production_per_product_periods($year,1,$month,$agent["ap_agents_code"],$section_sql,$section_way,'%%',$other_sql));
			
			while ($spremium = $sybase->fetch_assoc($ssql)) {
				
				$data[$agent["ap_agents_ID"]][$section]["premium"] += $spremium["clo_ytd_premium"];
				$data[$agent["ap_agents_ID"]][$section]["fees"] += $spremium["clo_period_fees"];
				$data[$agent["ap_agents_ID"]][$section]["commission"] += $spremium["clo_commission"];
				$data[$agent["ap_agents_ID"]][$section]["stamps"] += $spremium["clo_period_stamps"];
				$data[$agent["ap_agents_ID"]][$section]["mif"] += $spremium["clo_period_mif"];
			...
			$sql = "SELECT SUM(acl_ekremis)as ekremis , SUM(acl_pliromes)as pliromes, COUNT(acl_claims_ID)as claims FROM ap_claims WHERE acl_agent_ID = ".$agent["ap_agents_code_old"]." ".$section_sql." AND acl_year = '".$year."' AND acl_month = '".$month."'";
			$claim = $db->query_fetch($sql);
			$data[$agent["ap_agents_ID"]][$section]["ekremis"] = $claim["ekremis"];
			$data[$agent["ap_agents_ID"]][$section]["pliromes"] = $claim["pliromes"];
			$data[$agent["ap_agents_ID"]][$section]["claims"] = $claim["claims"];
			...
			$extra_sql = "AND inclm_open_date >= '".$year."-01-01' AND inclm_open_date <= '".$as_at_date."' ".$other_sql;
			//$extra_sql .= "AND clo_process_status in ('O','R','C')";
			$querysql = $inqueries->claims_oustanding_payments($as_at_date,1,$agent["ap_agents_code"],$section_sql,$section_way,$extra_sql);
			...
			while ($sclaim = $sybase->fetch_assoc($ssql)) {
				...
				$data[$agent["ap_agents_ID"]][$section]["claims"]++;
				$data[$agent["ap_agents_ID"]][$section]["pliromes"] += $sclaim["clo_amount_paid"];
				$data[$agent["ap_agents_ID"]][$section]["ekremis"] += $sclaim["clo_estimated_reserve"] - $sclaim["clo_amount_paid"];
