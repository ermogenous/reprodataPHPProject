...
if ($_GET["agent"] != "") {
$i=0;
for ($month = $_GET["month1"] ; $month <= $_GET["month2"]; $month++) {
$i++;
//information section
if ($_GET["action"] == "show") {
...
/*
	...
	$sql = "SELECT * FROM `ap_prod_comm` WHERE (`ap_agent` = '".$agent["inag_agent_code"]."' OR `ap_agent` = '".$agent["inag_numeric_key1"]."') AND `month` = ".$month." AND (`year` = ".$_GET["year1"]." OR `year` = ".$_GET["year2"]." OR `year` = ".$_GET["year3"].")";
	$result = $db->query($sql);

	while ($row = $db->fetch_assoc($result)) {
		
		//when group months
			$data[$row["klados"]][$row["year"]][$month]["production"] += $row["production"];
			$data[$row["klados"]][$row["year"]][$month]["comission"] += $row["comission"];
	
			$data["all"][$row["year"]][$month]["production"] += $row["production"];
			$data["all"][$row["year"]][$month]["comission"] += $row["comission"];
			
			$data[$row["klados"]][$row["year"]]["all"]["production"] += $row["production"];
			$data[$row["klados"]][$row["year"]]["all"]["comission"] += $row["comission"];

			$data["all"][$row["year"]]["all"]["production"] += $row["production"];
			$data["all"][$row["year"]]["all"]["comission"] += $row["comission"];
			

	}//WHILE production and comission
	
	$sql2 = "SELECT SUM(ekremis) AS `ekremis` , SUM(pliromes) AS `pliromes` , agent_ID , klados, month,year FROM `ap_claims` WHERE (`agent_ID` = '".$agent["inag_numeric_key1"]."' OR `agent_ID` = '".$agent["inag_agent_code"]."') AND `month` = ".$month." AND (`year` = ".$_GET["year1"]." OR `year` = ".$_GET["year2"]." OR `year` = ".$_GET["year3"].") GROUP BY klados,year,month";
echo "<HR>";
	...
	while ($claim = $db->fetch_assoc($result2)) {
		echo $claim["ekremis"]." - ";
		$data[$claim["klados"]][$claim["year"]][$month]["pliromes"] += $claim["pliromes"];
		$data[$claim["klados"]][$claim["year"]][$month]["ekremis"] += $claim["ekremis"];
		
		//$data["all"][$claim["year"]][$month]["pliromes"] += $claim["pliromes"];
		//$data["all"][$claim["year"]][$month]["ekremis"] += $claim["ekremis"];
		
		//$data[$claim["klados"]][$claim["year"]]["all"]["pliromes"] += $claim["pliromes"];
		//$data[$claim["klados"]][$claim["year"]]["all"]["ekremis"] += $claim["ekremis"];

		//$data["all"][$claim["year"]]["all"]["pliromes"] += $claim["pliromes"];
		//$data["all"][$claim["year"]]["all"]["ekremis"] += $claim["ekremis"];
...
for ($i=1;$i<4;$i++) {
	...
	$sql = $sqlpart . "AND (inped_year*100+inped_period)>=(".$_GET["year".$i]."*100+".$month.") 
AND (inped_year*100+inped_period)<=(".$_GET["year".$i]."*100+".$month.") 
    ...
    <td align="right"><table width="698" border="1" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC" class="main_text" style="border:solid #0000FF 1px">
      <tr>
        <td height="30" colspan="11" align="center"><strong>Agent : <? echo $agent["inag_agent_code"]." - ".$agent["inag_long_description"];?> For
          <? if ($_GET["group"] == 1){ echo $month_names[$_GET["month1"]]." TO ".$month_names[$month]; } else {echo $month_names[$month];}?>
        </strong></td>
      </tr>
      <tr>
...
<?
$all_products[0]["name"] = "Accident";
$all_products[0]["code"] = 10;
$all_products[1]["name"] = "Marine";
$all_products[1]["code"] = 16;
$all_products[2]["name"] = "Fire";
$all_products[2]["code"] = 17;
$all_products[3]["name"] = "Motor";
$all_products[3]["code"] = 19;
$all_products[4]["name"] = "Yacht";
$all_products[4]["code"] = 21;
$all_products[5]["name"] = "Liability";
$all_products[5]["code"] = 22;
...
?>	  
      <tr>
        <td height="17"><strong><? echo $all_products[$i]["code"]." ".$all_products[$i]["name"];?></strong></td>
        <td width="63" align="center" bgcolor="#FFFFCC"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year1"]][$month]["production"],0);?></strong></td>
        <td colspan="2" rowspan="2" align="center">&nbsp;</td>
        <td width="63" align="center" bgcolor="#CEFFCE"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year2"]][$month]["production"],0);?></strong></td>
        <td colspan="2" rowspan="2" align="center">&nbsp;</td>
        <td width="63" align="center" bgcolor="#C1C1FF"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year3"]][$month]["production"],0);?></strong></td>
        <td colspan="2" rowspan="2" align="center">&nbsp;</td>
      </tr>
      <tr>
        <td height="17" align="left">&nbsp;&nbsp;Comission</td>
        <td align="center" bgcolor="#FFFFCC"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year1"]][$month]["comission"],0);?></strong></td>
        <td align="center" bgcolor="#CEFFCE"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year2"]][$month]["comission"],0);?></strong></td>
        <td align="center" bgcolor="#C1C1FF"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year3"]][$month]["comission"],0);?></strong></td>
      </tr>
      <tr>
        <td height="17" align="left">&nbsp;&nbsp;Claims Incurred </td>
        <td align="center" bgcolor="#FFFFCC"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year1"]][$month]["ekremis"] + $data[$all_products[$i]["code"]][$_GET["year1"]][$month]["pliromes"],0);?></strong></td>
        <td align="center"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year1"]][$month]["ekremis"],0);?></strong></td>
        <td align="center"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year1"]][$month]["pliromes"],0);?></strong></td>
        <td align="center" bgcolor="#CEFFCE"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year2"]][$month]["ekremis"] + $data[$all_products[$i]["code"]][$_GET["year2"]][$month]["pliromes"],0);?></strong></td>
        <td align="center"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year2"]][$month]["ekremis"],0);?></strong></td>
        <td align="center"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year2"]][$month]["pliromes"],0);?></strong></td>
        <td align="center" bgcolor="#C1C1FF"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year3"]][$month]["ekremis"] + $data[$all_products[$i]["code"]][$_GET["year3"]][$month]["pliromes"],0);?></strong></td>
        <td align="center"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year3"]][$month]["ekremis"],0);?></strong></td>
        <td align="center"><strong><? echo round($data[$all_products[$i]["code"]][$_GET["year3"]][$month]["pliromes"],0);?></strong></td>
      </tr>
<?
}
?>	  
      ...
      <tr>
        ...
        <td align="center" bgcolor="#FFFFCC"><strong><? echo round($data["all"][$_GET["year1"]][$month]["production"],0);?></strong></td>
        <td colspan="2" rowspan="2" align="center">&nbsp;</td>
        <td align="center" bgcolor="#CEFFCE"><strong><? echo round($data["all"][$_GET["year2"]][$month]["production"],0);?></strong></td>
        <td colspan="2" rowspan="2" align="center">&nbsp;</td>
        <td align="center" bgcolor="#C1C1FF"><strong><? echo round($data["all"][$_GET["year3"]][$month]["production"],0);?></strong></td>
        <td colspan="2" rowspan="2" align="center">&nbsp;</td>
      </tr>
      <tr>
        <td height="17" align="left"><strong>Total Comission </strong></td>
        <td align="center" bgcolor="#FFFFCC"><strong><? echo round($data["all"][$_GET["year1"]][$month]["comission"],0);?></strong></td>
        <td align="center" bgcolor="#CEFFCE"><strong><? echo round($data["all"][$_GET["year2"]][$month]["comission"],0);?></strong></td>
        <td align="center" bgcolor="#C1C1FF"><strong><? echo round($data["all"][$_GET["year3"]][$month]["comission"],0);?></strong></td>
      </tr>
      <tr>
        <td height="17" align="left"><strong>Claims Incured </strong></td>
        <td align="center" bgcolor="#FFFFCC"><strong><? echo round($data["all"][$_GET["year1"]][$month]["ekremis"] + $data["all"][$_GET["year1"]][$month]["pliromes"],0);?></strong></td>
        <td align="center"><strong><? echo round($data["all"][$_GET["year1"]][$month]["ekremis"],2);?></strong></td>
        <td align="center"><strong><? echo round($data["all"][$_GET["year1"]][$month]["pliromes"],2);?></strong></td>
        <td align="center" bgcolor="#CEFFCE"><strong><? echo round($data["all"][$_GET["year2"]][$month]["ekremis"] + $data["all"][$_GET["year2"]][$month]["pliromes"],0);?></strong></td>
        <td align="center"><strong><? echo round($data["all"][$_GET["year2"]][$month]["ekremis"],0);?></strong></td>
        <td align="center"><strong><? echo round($data["all"][$_GET["year2"]][$month]["pliromes"],0);?></strong></td>
        <td align="center" bgcolor="#C1C1FF"><strong><? echo round($data["all"][$_GET["year3"]][$month]["ekremis"] + $data["all"][$_GET["year3"]][$month]["pliromes"],0);?></strong></td>
        <td align="center"><strong><? echo round($data["all"][$_GET["year3"]][$month]["ekremis"],0);?></strong></td>
        <td align="center"><strong><? echo round($data["all"][$_GET["year3"]][$month]["pliromes"],0);?></strong></td>
      </tr>
    </table></td>
  </tr>
</table>
